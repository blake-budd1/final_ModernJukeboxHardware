    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            if (event.type == sf::Event::Closed)
                ...
                nextSprite.setPosition(sf::Vector2f (albumSpriteEnd + rect.width/4, sf::VideoMode::getDesktopMode().height/2 - rect.height/4));

                // starting position
                int albumSpriteStart = sf::VideoMode::getDesktopMode().width/2 - (rect.width * 0.7);
 
                previousSprite.setPosition(sf::Vector2f (albumSpriteStart - rect.width/4, sf::VideoMode::getDesktopMode().height/2 - rect.height/4));

                // set position and sizes of backgrounds for next and previous:
                next_cover_background.setSize(sf::Vector2f(rect.width * 0.65, rect.height * 0.65));
                next_cover_background.setPosition(sf::Vector2f(albumSpriteEnd + rect.width/4.4, sf::VideoMode::getDesktopMode().height/2 - rect.height/3.625));
              
                prev_cover_background.setSize(sf::Vector2f(rect.width * 0.65, rect.height * 0.65));
                prev_cover_background.setPosition(sf::Vector2f(albumSpriteStart - rect.width/3.6, sf::VideoMode::getDesktopMode().height/2 - rect.height/3.625));
