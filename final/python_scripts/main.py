    ...
    if queueSize > 0 :
        ...
        with open(NEXT_SONG_PATH, "w") as file:
            if aquire_lock(file):
                ...
                file.write(os.path.join("../album_covers", image) + "\n")
        ...
        # Seems like this might be causing duplicates
        # for item in queue:
...
def Start():
    ...
    if queueSize >= 2:
        ...
        with open(NEXT_SONG_PATH, "w") as file:
            file.write(nextSong["uri"] + "\n")
            file.write(os.path.join("../album_covers", image_next) + "\n")
            file.write(nextSong["userAccessToken"] + "\n")
            file.write(nextSong["trackName"] + "\n")
            file.write(nextSong["trackArtist"] + "\n")
            file.write(nextSong["trackCover"] + "\n")
...
def updateFrontEndSong():
    # grab most recent song from previous
    fs = open('../temp_files/current_song.txt')
    songs = fs.readlines()
    fs.close()
    previous = songs[-8:]
    previous = [i.replace('\n', '') for i in previous]
    ...
    # convert previous song into json format
    dict_prev = {
        'uri': previous[0],
        'userAccessToken': previous[2],
        'duration': int(previous[6]),
        'trackName': previous[3],
        'trackArtist': previous[4],
        'trackCover': previous[5],
    }    

    for key, value in dict_prev.items():
        print(key, ": ", value)
    
    postData = {"message":dict_prev}
    postResponse = requests.post(CURR_URL, json=postData, headers={'Content-Type':'application/json'})
...
def queueFileController(command, previousCount):
    if command == "next" and previousCount == 0:
       ...
       # Start handles writing next_song.txt to next_songs.txt
        ...
        with open(NEXT_SONG_PATH, "w") as file:
            file.writelines(nextContent[:8])
       # Trying update previous here => should update the current playing song to the correct song on 
       # Front-end after user does a previous now
        updateFrontEndSong()
        updateSFML()
    ...
    while True:
        ...
        while True:
            ...
            if elapsedTime >= current_sleep_time + pausedOffset or command == "next":
                ...
                print("either command next or time for next song.")
                command = "next" # in the case that the song times out, we need to set the command to next manually
                updateSpotifyFiles()
                break
            ...
            if nextSongGet == 300 and nextSongBool == False:
                # Set the nextSongBool to get next song, if we have a next song, then this won't happen again until the next song is empty again
